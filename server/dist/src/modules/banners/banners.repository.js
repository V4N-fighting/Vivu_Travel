...
let BannersRepository = class BannersRepository {
    ...
    async findAll() {
        const query = 'SELECT id, text_content as "textContent", first_image as "firstImage", second_image as "secondImage", is_active as "isActive", sort_order as "sortOrder", page_location as "pageLocation" FROM banners ORDER BY sort_order ASC';
        const result = await this.pool.query(query);
        return result.rows;
    }
    async findActive() {
        const query = 'SELECT id, text_content as "textContent", first_image as "firstImage", second_image as "secondImage", is_active as "isActive", page_location as "pageLocation" FROM banners WHERE is_active = true ORDER BY sort_order ASC';
        const result = await this.pool.query(query);
        return result.rows;
    }
    async findByLocation(location) {
        const query = 'SELECT id, text_content as "textContent", first_image as "firstImage", second_image as "secondImage", is_active as "isActive", page_location as "pageLocation" FROM banners WHERE is_active = true AND page_location = $1 ORDER BY sort_order ASC';
