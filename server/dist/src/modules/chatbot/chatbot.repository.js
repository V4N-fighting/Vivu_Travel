...
var ChatbotRepository_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.ChatbotRepository = void 0;
const common_1 = require("@nestjs/common");
const pg_1 = require("pg");
const postgres_provider_1 = require("../../database/postgres.provider");
let ChatbotRepository = ChatbotRepository_1 = class ChatbotRepository {
    constructor(pool) {
        this.pool = pool;
        this.logger = new common_1.Logger(ChatbotRepository_1.name);
    }
    ...
    `);
        const tables = tablesResult.rows.reduce((acc, row) => {
        ...
        return {
            tables: Object.fromEntries(Object.entries(tables)
    ...
    async getOrCreateSession(sessionKey, userId) {
        await this.ensureRuntimeTables();
        const existing = await this.pool.query('SELECT * FROM ai_chat_sessions WHERE session_key = $1 LIMIT 1', [sessionKey]);
        if (existing.rows[0])
            return existing.rows[0];
        const created = await this.pool.query(`INSERT INTO ai_chat_sessions (session_key, user_id)
       VALUES ($1, $2)
       RETURNING *`, [sessionKey, userId || null]);
        return created.rows[0];
    }
    async getMessages(sessionId, limit = 12) {
        const result = await this.pool.query(`SELECT role, content
    ...
    async saveMessage(sessionId, role, content, metadata = {}) {
        await this.pool.query(`INSERT INTO ai_chat_messages (session_id, role, content, metadata)
       VALUES ($1, $2, $3, $4)`, [sessionId, role, content, metadata]);
    }
    async updateMemory(sessionId, memory, summary) {
        await this.pool.query(`UPDATE ai_chat_sessions
       SET memory = $2, summary = COALESCE($3, summary), updated_at = CURRENT_TIMESTAMP
       WHERE id = $1`, [sessionId, memory, summary || null]);
    }
    async logAnalytics(data) {
        try {
            await this.pool.query(`INSERT INTO ai_chat_analytics
         ...
         VALUES ($1, $2, $3, $4, $5, $6)`, [
            ...
            this.logger.warn(`Could not write chat analytics: ${error.message}`);
        }
    }
    async searchToursBySql(filters, limit = 8) {
        const params = [];
        let query = `
      SELECT
        t.id,
    ...
    async fullTextSearch(message, limit = 10) {
    ...
    async getTourReviewComments(tourId, limit = 8) {
        const result = await this.pool.query(`SELECT rating, comment FROM reviews WHERE tour_id = $1 AND comment IS NOT NULL AND comment != '' ORDER BY created_at DESC LIMIT $2`, [tourId, limit]);
        return result.rows.map((row) => `[Rating: ${row.rating}/5] ${row.comment}`);
    }
};
exports.ChatbotRepository = ChatbotRepository;
exports.ChatbotRepository = ChatbotRepository = ChatbotRepository_1 = __decorate([
