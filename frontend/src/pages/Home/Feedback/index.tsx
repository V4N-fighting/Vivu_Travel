...
// Dữ liệu mặc định khi chưa có review trong DB
const defaultReview: ReviewItem = {
  id: 0,
  firstName: "Lương",
  lastName: "Lã Lướt",
  avatar: "./images/avater-1-1.jpg",
  rating: 5,
  comment: "Quả là 1 nơi tuyệt vời để tìm kiếm 1 nơi nghỉ ngơi cho mình, tôi rất yêu nơi này.",
  tour_name: "Ha Long Bay Cruise"
};

const Feedback: React.FC = () => {
  const [review, setReview] = useState<ReviewItem>(defaultReview);

  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await axios.get(`${GET_REVIEW}/latest?limit=1`);
        if (res.data && res.data.length > 0) {
          setReview(res.data[0]);
        }
      } catch (error) {
        // Dùng dữ liệu mặc định nếu fail
        console.log("Dùng dữ liệu mặc định cho feedback");
      }
    };
    fetchReviews();
  }, []);
  ...
  const displayName = review.firstName && review.lastName 
    ...
    : review.firstName || defaultReview.lastName;
                ...
                <BorderLine>
                  <Contain>
                    ...
                    <FeedbackContain>
                      ...
                      <StarRating star={review.rating || 5} />
                      <TextContain>
                        {review.comment || defaultReview.comment}
                      </TextContain>
                      <CurAvatar main url={review.avatar || './images/avater-1-1.jpg'} ></CurAvatar>
